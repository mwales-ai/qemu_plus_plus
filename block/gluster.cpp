                                  ...
                                  const char *filename)
{
    ...
    if (is_unix) {
        char *uri_socket = static_cast<char *>(g_hash_table_lookup(qp, "socket"));
                                  ...
                                  QDict *options, Error **errp)
{
    ...
    for (i = 0; i < num_servers; i++) {
        ...
        gsconf->type = static_cast<SocketAddressType>(type);
        qemu_opts_del(opts);
...
{
    BDRVGlusterState *s = static_cast<BDRVGlusterState *>(bs->opaque);
    int open_flags = 0;
    int ret = 0;
                                       ...
                                       BlockReopenQueue *queue, Error **errp)
{
    ...
    s = static_cast<BDRVGlusterState *>(state->bs->opaque);

    state->opaque = g_new0(BDRVGlusterReopenState, 1);
    reop_s = static_cast<BDRVGlusterReopenState *>(state->opaque);
...
{
    BDRVGlusterReopenState *reop_s = static_cast<BDRVGlusterReopenState *>(state->opaque);
    BDRVGlusterState *s = static_cast<BDRVGlusterState *>(state->bs->opaque);
                                                    ...
                                                    Error **errp)
{
    ...
    gopts->preallocation = static_cast<PreallocMode>(qapi_enum_parse(&PreallocMode_lookup, tmp,
                                                 ...
                                                 int64_t offset, int64_t bytes)
{
    ...
    assert(static_cast<uint64_t>(bytes) <= SIZE_MAX); /* rely on max_pdiscard */
