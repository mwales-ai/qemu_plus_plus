...
cor_open(BlockDriverState *bs, QDict *options, int flags, Error **errp)
{
    ...
    BDRVStateCOR *state = static_cast<BDRVStateCOR *>(bs->opaque);
    ...
    while (bytes) {
        ...
        if (ret <= 0) {
            ret = bdrv_co_is_allocated_above(bdrv_backing_chain_next(bs->file->bs),
                                             ...
                                             n, &n);
            if (ret > 0 || ret < 0) {
                local_flags = local_flags | BDRV_REQ_COPY_ON_READ;
            }
...
{
    BDRVStateCOR *s = static_cast<BDRVStateCOR *>(bs->opaque);
...
{
    BDRVStateCOR *s = static_cast<BDRVStateCOR *>(cor_filter_bs->opaque);
