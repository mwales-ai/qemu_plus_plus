...
/* Forward declarations of callback functions for backup_job_driver */
...
{
    BackupBlockJob *s = static_cast<BackupBlockJob *>(opaque);
...
static int coroutine_fn backup_run(Job *job, Error **errp)
{
    ...
    if (s->sync_mode == MIRROR_SYNC_MODE_TOP) {
        ...
        for (offset = 0; offset < static_cast<int64_t>(s->len); ) {
                  ...
                  JobTxn *txn, Error **errp)
{
    ...
    job = static_cast<BackupBlockJob *>(
              block_job_create(job_id, &backup_job_driver, txn, cbw,
                               0, BLK_PERM_ALL,
