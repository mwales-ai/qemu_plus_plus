...
static int preallocate_open(BlockDriverState *bs, QDict *options, int flags,
                            Error **errp)
{
    BDRVPreallocateState *s = static_cast<BDRVPreallocateState *>(bs->opaque);
    int ret;
...
preallocate_truncate_to_real_size(BlockDriverState *bs, Error **errp)
{
    ...
    if (s->data_end < s->file_end) {
        ret = bdrv_truncate(bs->file, s->data_end, true, PREALLOC_MODE_OFF,
                            static_cast<BdrvRequestFlags>(0), NULL);
        if (ret < 0) {
...
{
    BDRVPreallocateState *s = static_cast<BDRVPreallocateState *>(state->bs->opaque);

    s->opts = *static_cast<PreallocateOpts *>(state->opaque);
...
static void preallocate_drop_resize_bh(void *opaque)
{
     ...
     * indefinitely.
     */
    preallocate_drop_resize(static_cast<BlockDriverState *>(opaque), NULL);
}
