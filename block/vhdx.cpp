...
bool vhdx_checksum_is_valid(uint8_t *buf, size_t size, int crc_offset)
{
    ...
    assert(buf != NULL);
    assert(size > (size_t)(crc_offset + 4));
                                 ...
                                 uint64_t start, uint64_t length)
{
    ...
    r = static_cast<VHDXRegionEntry *>(g_malloc0(sizeof(*r)));
                             ...
                             uint64_t offset, bool read)
{
    ...
    buffer = static_cast<uint8_t *>(qemu_blockalign(bs_file, VHDX_HEADER_SIZE));
    if (read) {
        /* if true, we can't assume the extra reserved bytes are 0 */
        ret = bdrv_pread(file, offset, VHDX_HEADER_SIZE, buffer, static_cast<BdrvRequestFlags>(0));
        if (ret < 0) {
            goto exit;
        }
    } else {
    ...
    ret = bdrv_pwrite_sync(file, offset, sizeof(VHDXHeader), header_le, static_cast<BdrvRequestFlags>(0));
...
vhdx_parse_header(BlockDriverState *bs, BDRVVHDXState *s, Error **errp)
{
    ...
    header1 = static_cast<VHDXHeader *>(qemu_blockalign(bs, sizeof(VHDXHeader)));
    header2 = static_cast<VHDXHeader *>(qemu_blockalign(bs, sizeof(VHDXHeader)));

    buffer = static_cast<uint8_t *>(qemu_blockalign(bs, VHDX_HEADER_SIZE));
...
vhdx_open_region_tables(BlockDriverState *bs, BDRVVHDXState *s)
{
     ...
     * whole block */
    buffer = static_cast<uint8_t *>(qemu_blockalign(bs, VHDX_HEADER_BLOCK_SIZE));

    ret = bdrv_pread(bs->file, VHDX_REGION_TABLE_OFFSET,
                     VHDX_HEADER_BLOCK_SIZE, buffer, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto fail;
    }
...
vhdx_parse_metadata(BlockDriverState *bs, BDRVVHDXState *s)
{
    ...
    buffer = static_cast<uint8_t *>(qemu_blockalign(bs, VHDX_METADATA_TABLE_MAX_SIZE));

    ret = bdrv_pread(bs->file, s->metadata_rt.file_offset,
                     VHDX_METADATA_TABLE_MAX_SIZE, buffer, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto exit;
    }
    ...
    if ((s->metadata_hdr.entry_count * sizeof(md_entry)) >
        (size_t)(VHDX_METADATA_TABLE_MAX_SIZE - offset)) {
        ret = -EINVAL;
        goto exit;
    }
...
{
    BDRVVHDXState *s = static_cast<BDRVVHDXState *>(bs->opaque);
    ...
    for (i = 0; i < s->bat_entries; i++) {
        if ((s->bat[i] & VHDX_BAT_STATE_BIT_MASK) ==
             ...
             */
            if (offset > (uint64_t)(INT64_MAX - s->block_size)) {
             ...
             */
            if (offset >= (uint64_t)image_file_size) {
                ...
                (*errcnt)++;
            } else if (offset + block_length > (uint64_t)image_file_size) {
                     ...
                     Error **errp)
{
    ...
    ret = bdrv_pread(bs->file, 0, sizeof(uint64_t), &signature, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto fail;
    }
    ...
    s->bat = static_cast<VHDXBatEntry *>(qemu_try_blockalign(bs->file->bs, s->bat_rt.length));
    if (s->bat == NULL) {
        ret = -ENOMEM;
        goto fail;
    }

    ret = bdrv_pread(bs->file, s->bat_offset, s->bat_rt.length, s->bat, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto fail;
    }
                                 ...
                                 int nb_sectors, VHDXSectorInfo *sinfo)
{
    ...
    if (sinfo->sectors_avail > (uint32_t)nb_sectors) {
        ...
        if (s->params.data_bits & VHDX_PARAMS_HAS_PARENT) {
            ...
            case PAYLOAD_BLOCK_FULLY_PRESENT:
                ...
                ret = bdrv_co_preadv(bs->file, sinfo.file_offset,
                                     ...
                                     &hd_qiov, static_cast<BdrvRequestFlags>(0));
                qemu_co_mutex_lock(&s->lock);
                if (ret < 0) {
                    goto exit;
                }
                    ...
                    uint64_t *new_offset, bool *need_zero)
{
    ...
    *new_offset = ROUND_UP(*new_offset, 1 * MiB);
    ...
    return bdrv_co_truncate(bs->file, *new_offset + s->block_size, false,
                            PREALLOC_MODE_OFF, static_cast<BdrvRequestFlags>(0), NULL);
}
                         ...
                         uint64_t metadata_offset, VHDXImageType type)
{
    ...
    mt_file_params = static_cast<VHDXFileParameters *>(entry_buffer);
    offset += sizeof(VHDXFileParameters);
    mt_virtual_size = reinterpret_cast<VHDXVirtualDiskSize *>(static_cast<char *>(entry_buffer) + offset);
    offset += sizeof(VHDXVirtualDiskSize);
    mt_page83 = reinterpret_cast<VHDXPage83Data *>(static_cast<char *>(entry_buffer) + offset);
    offset += sizeof(VHDXPage83Data);
    mt_log_sector_size = reinterpret_cast<VHDXVirtualDiskLogicalSectorSize *>(static_cast<char *>(entry_buffer) + offset);
    offset += sizeof(VHDXVirtualDiskLogicalSectorSize);
    mt_phys_sector_size = reinterpret_cast<VHDXVirtualDiskPhysicalSectorSize *>(static_cast<char *>(entry_buffer) + offset);
    ...
    md_table = static_cast<VHDXMetadataTableHeader *>(buffer);
    ...
    md_table_entry = reinterpret_cast<VHDXMetadataTableEntry *>(static_cast<char *>(buffer) + sizeof(VHDXMetadataTableHeader));
    ...
    ret = blk_co_pwrite(blk, metadata_offset, VHDX_HEADER_BLOCK_SIZE, buffer, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto exit;
    }

    ret = blk_co_pwrite(blk, metadata_offset + (64 * KiB),
                        VHDX_METADATA_ENTRY_BUFFER_SIZE, entry_buffer, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto exit;
    }
                ...
                uint32_t length, Error **errp)
{
    ...
    if (type == VHDX_TYPE_DYNAMIC) {
        ...
        ret = blk_co_truncate(blk, data_file_offset + image_size, false,
                              PREALLOC_MODE_OFF, static_cast<BdrvRequestFlags>(0), errp);
        if (ret < 0) {
            goto exit;
        }
    } else {
    ...
    if (type == VHDX_TYPE_FIXED ||
        ...
        s->bat = static_cast<VHDXBatEntry *>(g_try_malloc0(length));
        ...
        ret = blk_co_pwrite(blk, file_offset, length, s->bat, static_cast<BdrvRequestFlags>(0));
        if (ret < 0) {
                             ...
                             Error **errp)
{
     ...
     * entire 64KB block */
    buffer = g_malloc0(VHDX_HEADER_BLOCK_SIZE);
    region_table = static_cast<VHDXRegionTableHeader *>(buffer);
    offset += sizeof(VHDXRegionTableHeader);
    rt_bat = reinterpret_cast<VHDXRegionTableEntry *>(static_cast<char *>(buffer) + offset);
    offset += sizeof(VHDXRegionTableEntry);
    rt_metadata  = reinterpret_cast<VHDXRegionTableEntry *>(static_cast<char *>(buffer) + offset);
    ...
    *metadata_offset = rt_metadata->file_offset;
    ...
    vhdx_update_checksum(static_cast<uint8_t *>(buffer), VHDX_HEADER_BLOCK_SIZE,
    ...
    ret = blk_co_pwrite(blk, VHDX_REGION_TABLE_OFFSET, VHDX_HEADER_BLOCK_SIZE,
                        buffer, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
...
vhdx_co_create(BlockdevCreateOptions *opts, Error **errp)
{
    ...
    if (creator) {
        ret = blk_co_pwrite(blk, VHDX_FILE_ID_OFFSET + sizeof(signature),
                            creator_items * sizeof(gunichar2), creator, static_cast<BdrvRequestFlags>(0));
        if (ret < 0) {
