                            ...
                            int64_t *requested_bytes, void *buf)
{
    ...
    if (ret & BDRV_BLOCK_ALLOCATED) {
        if (ret & BDRV_BLOCK_ZERO) {
            ...
            assert(bytes < static_cast<int64_t>(SIZE_MAX));

            ret = blk_co_pread(s->top, offset, bytes, buf, static_cast<BdrvRequestFlags>(0));
            if (ret < 0) {
                goto fail;
            }

            ret = blk_co_pwrite(s->base, offset, bytes, buf, static_cast<BdrvRequestFlags>(0));
            if (ret < 0) {
                error_in_source = false;
                goto fail;
            }
...
static int coroutine_fn commit_run(Job *job, Error **errp)
{
    ...
    if (base_len < len) {
        ret = blk_co_truncate(s->base, len, false, PREALLOC_MODE_OFF, static_cast<BdrvRequestFlags>(0), NULL);
        if (ret) {
            return ret;
        }
    }
                  ...
                  const char *filter_node_name, Error **errp)
{
    ...
    s = static_cast<CommitBlockJob *>(block_job_create(job_id, &commit_job_driver, NULL, bs, 0, BLK_PERM_ALL,
...
int bdrv_commit(BlockDriverState *bs)
{
    ...
    if (length > backing_length) {
        ret = blk_truncate(backing, length, false, PREALLOC_MODE_OFF, static_cast<BdrvRequestFlags>(0),
                           &local_err);
        if (ret < 0) {
            error_report_err(local_err);
            goto ro_cleanup;
    ...
    buf = static_cast<uint8_t *>(blk_try_blockalign(src, COMMIT_BUF_SIZE));
    if (buf == NULL) {
        ret = -ENOMEM;
        goto ro_cleanup;
    }
    ...
    for (offset = 0; offset < length; offset += n) {
        ...
        if (ret) {
            ret = blk_pread(src, offset, n, buf, static_cast<BdrvRequestFlags>(0));
            if (ret < 0) {
                goto ro_cleanup;
            }

            ret = blk_pwrite(backing, offset, n, buf, static_cast<BdrvRequestFlags>(0));
            if (ret < 0) {
                goto ro_cleanup;
            }
        }
