...
{
    int ret;
    BDRVReplicationState *s = static_cast<BDRVReplicationState *>(bs->opaque);
    QemuOpts *opts = NULL;
    const char *mode;
    const char *top_id;
                     ...
                     int remaining_sectors, QEMUIOVector *qiov)
{
    ...
    ret = bdrv_co_preadv(bs->file, sector_num * BDRV_SECTOR_SIZE,
                         remaining_sectors * BDRV_SECTOR_SIZE, qiov, static_cast<BdrvRequestFlags>(0));
                      ...
                      int remaining_sectors, QEMUIOVector *qiov, int flags)
{
    ...
    while (remaining_sectors > 0) {
        ...
        ret = bdrv_co_pwritev(target, sector_num * BDRV_SECTOR_SIZE,
                              n * BDRV_SECTOR_SIZE, &hd_qiov, static_cast<BdrvRequestFlags>(0));
        if (ret < 0) {
            goto out1;
        }
...
static void replication_start(ReplicationState *rs, ReplicationMode mode,
                              Error **errp)
{
    BlockDriverState *bs = static_cast<BlockDriverState *>(rs->opaque);
    ...
    s = static_cast<BDRVReplicationState *>(bs->opaque);
    ...
    case REPLICATION_MODE_SECONDARY:
        ...
        s->backup_job = backup_job_create(
                                ...
                                0, MIRROR_SYNC_MODE_NONE, NULL, static_cast<BitmapSyncMode>(0), false, false,
                                NULL, &perf,
