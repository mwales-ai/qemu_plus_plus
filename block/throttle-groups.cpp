...
{
    RestartData *data = static_cast<RestartData *>(opaque);
...
void throttle_group_restart_tgm(ThrottleGroupMember *tgm)
{
    ...
    if (tgm->throttle_state) {
        for (dir = THROTTLE_READ; dir < THROTTLE_MAX; dir = static_cast<ThrottleDirection>(dir + 1)) {
...
{
    timer_cb(static_cast<ThrottleGroupMember *>(opaque), THROTTLE_READ);
}

static void write_timer_cb(void *opaque)
{
    timer_cb(static_cast<ThrottleGroupMember *>(opaque), THROTTLE_WRITE);
}
...
typedef struct {
    const char *name;
    BucketType type;
    ThrottleParamCategory category;
} ThrottleParamInfo;
...
static ThrottleParamInfo properties[] = {
    ...
    },
    {
        ...
        static_cast<BucketType>(0), IOPS_SIZE,
    }
};
...

{
    ThrottleGroup *tg = THROTTLE_GROUP(obj);
    ThrottleConfig *cfg;
    ThrottleParamInfo *info = static_cast<ThrottleParamInfo *>(opaque);
    int64_t value;
...
static const InterfaceInfo throttle_group_interfaces[] = {
    { TYPE_USER_CREATABLE },
    { }
};
