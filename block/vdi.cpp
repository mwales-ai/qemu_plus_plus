...
extern QemuOptsList vdi_create_opts;
                                     ...
                                     BdrvCheckMode fix)
{
    ...
    BDRVVdiState *s = static_cast<BDRVVdiState *>(bs->opaque);
...
static int vdi_probe(const uint8_t *buf, int buf_size, const char *filename)
{
    ...
    if ((size_t)buf_size < sizeof(*header)) {
                    ...
                    Error **errp)
{
    ...
    s->bmap = static_cast<uint32_t *>(qemu_try_blockalign(bs->file->bs, bmap_size * SECTOR_SIZE));
    if (s->bmap == NULL) {
        ret = -ENOMEM;
        goto fail;
    }
    ...
    ret = bdrv_pread(bs->file, header.offset_bmap, bmap_size * SECTOR_SIZE,
                     s->bmap, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
        goto fail_free_bmap;
    }
    ...
    while (ret >= 0 && bytes > 0) {
        ...
        if (!VDI_IS_ALLOCATED(bmap_entry)) {
            ...
            data_offset = s->header.offset_data +
                          (uint64_t)bmap_entry * s->block_size;
            if (block == NULL) {
                block = static_cast<uint8_t *>(g_malloc(s->block_size));
...
nonallocating_write:
    ...
    if (block) {
        ...
        g_free(block);
        header = static_cast<VdiHeader *>(g_malloc(sizeof(*header)));

        logout("now writing modified header\n");
        assert(VDI_IS_ALLOCATED(bmap_first));
        *header = s->header;
        vdi_header_to_le(header);
        ret = bdrv_co_pwrite(bs->file, 0, sizeof(*header), header, static_cast<BdrvRequestFlags>(0));
        g_free(header);
        ...
        ret = bdrv_co_pwrite(bs->file, bmap_offset * SECTOR_SIZE,
                             n_sectors * SECTOR_SIZE, base, static_cast<BdrvRequestFlags>(0));
    }
                 ...
                 Error **errp)
{
    ...
    vdi_header_to_le(&header);
    ret = blk_co_pwrite(blk, offset, sizeof(header), &header, static_cast<BdrvRequestFlags>(0));
    if (ret < 0) {
    ...
    if (bmap_size > 0) {
        bmap = static_cast<uint32_t *>(g_try_malloc0(bmap_size));
        if (bmap == NULL) {
            ret = -ENOMEM;
        ...
        ret = blk_co_pwrite(blk, offset, bmap_size, bmap, static_cast<BdrvRequestFlags>(0));
        if (ret < 0) {
