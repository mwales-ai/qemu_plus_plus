...
BlockExport *blk_exp_add(BlockExportOptions *blk_export, Error **errp)
{
    bool fixed_iothread = blk_export->has_fixed_iothread && blk_export->fixed_iothread;
    bool allow_inactive = blk_export->has_allow_inactive && blk_export->allow_inactive;
    ...
    if (!id_wellformed(blk_export->id)) {
        error_setg(errp, "Invalid block export id");
        return NULL;
    }
    if (blk_exp_find(blk_export->id)) {
        error_setg(errp, "Block export id '%s' is already in use", blk_export->id);
        return NULL;
    }

    drv = blk_exp_find_driver(blk_export->type);
    if (!drv) {
        error_setg(errp, "No driver found for the requested export type");
        return NULL;
    }

    bs = bdrv_lookup_bs(NULL, blk_export->node_name, errp);
    if (!bs) {
        return NULL;
    }

    if (!blk_export->has_writable) {
        blk_export->writable = false;
    }
    if (bdrv_is_read_only(bs) && blk_export->writable) {
    ...
    if (blk_export->iothread) {
        IOThread *iothread;
        AioContext *new_ctx;
        Error **set_context_errp;

        iothread = iothread_by_id(blk_export->iothread);
        if (!iothread) {
            error_setg(errp, "iothread \"%s\" not found", blk_export->iothread);
            goto fail;
        }
    ...
    if (!blk_export->has_writethrough) {
        blk_export->writethrough = false;
    }
    blk_set_enable_write_cache(blk, !blk_export->writethrough);

    assert(drv->instance_size >= sizeof(BlockExport));
    exp = static_cast<BlockExport *>(g_malloc0(drv->instance_size));
    memset(exp, 0, drv->instance_size);
    exp->drv = drv;
    exp->refcount = 1;
    exp->user_owned = true;
    exp->id = g_strdup(blk_export->id);
    exp->ctx = ctx;
    exp->blk = blk;

    ret = drv->create(exp, blk_export, errp);
    if (ret < 0) {
        goto fail;
    }
...
{
    BlockExport *exp = static_cast<BlockExport *>(opaque);
...
void qmp_block_export_add(BlockExportOptions *blk_export, Error **errp)
{
    blk_exp_add(blk_export, errp);
}
...
BlockExportInfoList *qmp_query_block_exports(Error **errp)
{
    ...
    QLIST_FOREACH(exp, &block_exports, next) {
        ...
        info->id = g_strdup(exp->id);
        info->type = exp->drv->type;
        info->node_name = g_strdup(bdrv_get_node_name(blk_bs(exp->blk)));
        info->shutting_down = !exp->user_owned;
