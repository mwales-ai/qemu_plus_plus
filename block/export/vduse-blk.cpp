...
static void vduse_blk_inflight_dec(VduseBlkExport *vblk_exp)
{
    if (qatomic_fetch_dec(&vblk_exp->inflight) == 1) {
        ...
        blk_exp_unref(&vblk_exp->blk_exp);
    }
}
...
{
    VduseBlkReq *req = static_cast<VduseBlkReq *>(opaque);
    VduseVirtq *vq = req->vq;
    VduseDev *dev = vduse_queue_get_dev(vq);
    VduseBlkExport *vblk_exp = static_cast<VduseBlkExport *>(vduse_dev_get_priv(dev));
...
static void vduse_blk_vq_handler(VduseDev *dev, VduseVirtq *vq)
{
    ...
    while (1) {
        ...
        req = static_cast<VduseBlkReq *>(vduse_queue_pop(vq, sizeof(VduseBlkReq)));
        if (!req) {
            break;
        }
        req->vq = vq;
...
{
    VduseVirtq *vq = static_cast<VduseVirtq *>(opaque);
    VduseDev *dev = vduse_queue_get_dev(vq);
...
static void vduse_blk_enable_queue(VduseDev *dev, VduseVirtq *vq)
{
    ...
    aio_set_fd_handler(vblk_exp->blk_exp.ctx, vduse_queue_get_fd(vq),
...
static void vduse_blk_disable_queue(VduseDev *dev, VduseVirtq *vq)
{
    ...
    aio_set_fd_handler(vblk_exp->blk_exp.ctx, fd,
...
{
    VduseDev *dev = static_cast<VduseDev *>(opaque);
...
{
    aio_set_fd_handler(vblk_exp->blk_exp.ctx, vduse_dev_get_fd(vblk_exp->dev),
...
{
    VduseBlkExport *vblk_exp = static_cast<VduseBlkExport *>(opaque);

    vblk_exp->blk_exp.ctx = ctx;
...
static void blk_aio_detach(void *opaque)
{
    ...
    vblk_exp->blk_exp.ctx = NULL;
}

static void vduse_blk_resize(void *opaque)
{
    BlockExport *exp = static_cast<BlockExport *>(opaque);
    VduseBlkExport *vblk_exp = container_of(exp, VduseBlkExport, blk_exp);
