...
{
    VuBlkReq *req = static_cast<VuBlkReq *>(opaque);
...
static void vu_blk_process_vq(VuDev *vu_dev, int idx)
{
    ...
    while (1) {
        ...
        req = static_cast<VuBlkReq *>(vu_queue_pop(vu_dev, vq, sizeof(VuBlkReq)));
        if (!req) {
            break;
        }
                    ...
                    uint32_t offset, uint32_t size, uint32_t flags)
{
    ...
    wce = *data;
    vexp->blkcfg.wce = wce;
    blk_set_enable_write_cache(vexp->blk_exp.blk, wce);
    return 0;
}
...
{
    VuBlkExport *vexp = static_cast<VuBlkExport *>(opaque);

    /*
     * The actual attach will happen in vu_blk_drained_end() and we just
     * restore ctx here.
     */
    vexp->blk_exp.ctx = ctx;
}
...
{
    VuBlkExport *vexp = container_of(exp, VuBlkExport, blk_exp);
...
static void vu_blk_drained_end(void *opaque)
{
    ...
    vhost_user_server_attach_aio_context(&vexp->vu_server, vexp->blk_exp.ctx);
}
