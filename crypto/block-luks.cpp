...
#define qcrypto_block_luks_cipher_mode_lookup(name, errp)               \
    static_cast<QCryptoCipherMode>(qcrypto_block_luks_name_lookup(      \
                                   name,                                \
...
#define qcrypto_block_luks_hash_name_lookup(name, errp)                 \
    static_cast<QCryptoHashAlgo>(qcrypto_block_luks_name_lookup(        \
                                   name,                                \
...
#define qcrypto_block_luks_ivgen_name_lookup(name, errp)                \
    static_cast<QCryptoIVGenAlgo>(qcrypto_block_luks_name_lookup(       \
                                   name,                                \
...
qcrypto_block_luks_has_format(const uint8_t *buf,
                              size_t buf_size)
{
    const QCryptoBlockLUKSHeader *luks_header = reinterpret_cast<const QCryptoBlockLUKSHeader *>(buf);
    ...
    case QCRYPTO_CIPHER_ALGO_AES_256:
        if (digestlen == qcrypto_cipher_get_key_len(
            ...
            error_setg(errp, "No AES cipher with key size %zu available",
                       digestlen);
            return static_cast<QCryptoCipherAlgo>(0);
        }
        break;
...
qcrypto_block_luks_store_header(QCryptoBlock *block,
                                ...
                                Error **errp)
{
    const QCryptoBlockLUKS *luks = static_cast<const QCryptoBlockLUKS *>(block->opaque);
...
{
    int rv;
    QCryptoBlockLUKS *luks = static_cast<QCryptoBlockLUKS *>(block->opaque);
...
qcrypto_block_luks_parse_header(QCryptoBlockLUKS *luks, Error **errp)
{
    ...
    *ivgen_name = '\0';
    ...
    if (!ivhash_name) {
        luks->ivgen_hash_alg = static_cast<QCryptoHashAlgo>(0);
    } else {
        *ivhash_name = '\0';
        ivhash_name++;
    ...
    luks->cipher_alg =
            static_cast<QCryptoCipherAlgo>(qcrypto_block_luks_cipher_name_lookup(luks->header.cipher_name,
                                                  luks->cipher_mode,
