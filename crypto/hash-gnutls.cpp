...
QCryptoHash *qcrypto_gnutls_hash_new(QCryptoHashAlgo alg, Error **errp)
{
    ...
    ret = gnutls_hash_init(static_cast<gnutls_hash_hd_t *>(hash->opaque),
                           static_cast<gnutls_digest_algorithm_t>(qcrypto_hash_alg_map[alg]));
    if (ret < 0) {
        error_setg(errp,
...
{
    gnutls_hash_hd_t *ctx = static_cast<gnutls_hash_hd_t *>(hash->opaque);
                                 ...
                                 Error **errp)
{
    ...
    ret = gnutls_hash_get_len(static_cast<gnutls_digest_algorithm_t>(qcrypto_hash_alg_map[hash->alg]));
    if (ret == 0) {
