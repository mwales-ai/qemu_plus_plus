                           ...
                           Error **errp)
{
    ...
    err = gnutls_hmac_init(&ctx->handle,
                           static_cast<gnutls_mac_algorithm_t>(qcrypto_hmac_alg_map[alg]),
                           (const void *)key, nkey);
    if (err != 0) {
...
qcrypto_gnutls_hmac_ctx_free(QCryptoHmac *hmac)
{
    ...
    ctx = static_cast<QCryptoHmacGnutls *>(hmac->opaque);
                           ...
                           Error **errp)
{
    ...
    ret = gnutls_hmac_get_len(static_cast<gnutls_mac_algorithm_t>(qcrypto_hmac_alg_map[hmac->alg]));
    if (ret <= 0) {
