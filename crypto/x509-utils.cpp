                                      ...
                                      Error **errp)
{
    ...
    gnutls_datum_t datum = {.data = cert, .size = (unsigned int)size};
    ...
    hlen = gnutls_hash_get_len(static_cast<gnutls_digest_algorithm_t>(qcrypto_to_gnutls_hash_alg_map[alg]));
    if (*resultlen < (size_t)hlen) {
        error_setg(errp,
    ...
    if (gnutls_x509_crt_get_fingerprint(crt,
                                        static_cast<gnutls_digest_algorithm_t>(qcrypto_to_gnutls_hash_alg_map[alg]),
                                        result, resultlen) != 0) {
