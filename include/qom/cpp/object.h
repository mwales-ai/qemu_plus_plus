...
/*
 * REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_IFACES: abstract object type with
 * instance_size + interfaces. No class_init / instance_init / class_size.
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_IFACES(unique_tag, ClassName,    \
                                                    type_name_str,           \
                                                    parent_type_str,         \
                                                    ifaces_array)            \
static void unique_tag##_cpp_register_types(void)                            \
 ...
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_INIT_SIZED_IFACES(unique_tag, ClassName, \
                                                         type_name_str,       \
                                                         parent_type_str,     \
                                                         instance_init_fn,    \
                                                         ifaces_array)        \
...
/*
 * REGISTER_QEMU_OBJECT_ABSTRACT_INIT_CS_IFACES: abstract sized + free
 * instance_init + class_size + interfaces. No class_init.
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_INIT_CS_IFACES(                        \
    unique_tag, ClassName, ClassStruct, type_name_str, parent_type_str,      \
    instance_init_fn, ifaces_array)                                          \
static void unique_tag##_cpp_register_types(void)                            \
{                                                                            \
    static const TypeInfo info = {                                           \
        .name          = type_name_str,                                      \
 ...
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_CI_BARE(unique_tag,                    \
                                               type_name_str,                \
                                               parent_type_str,              \
                                               class_init_fn)                \
static void unique_tag##_cpp_register_types(void)                            \
 ...
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_CS_CI(unique_tag, ClassName,     \
                                                   ClassStruct,              \
                                                   type_name_str,            \
                                                   parent_type_str,          \
                                                   class_init_fn)            \
...
/*
 * REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_CI_IFACES: abstract sized object with
 * free class_init + interfaces. No class_size, no instance_init.
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_CI_IFACES(                       \
...
/*
 * REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_CS_CI_IFACES: abstract sized object
 * with class_size + class_init + interfaces.
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_SIZED_CS_CI_IFACES(                    \
    unique_tag, ClassName, ClassStruct, type_name_str, parent_type_str,      \
    class_init_fn, ifaces_array)                                             \
static void unique_tag##_cpp_register_types(void)                            \
{                                                                            \
 ...
 */
#define REGISTER_QEMU_OBJECT_INIT_CLASS_SIZED_CS(unique_tag, ClassName,      \
                                                  ClassStruct,               \
                                                  type_name_str,             \
                                                  parent_type_str,           \
                                                  instance_init_fn,          \
...
/*
 * REGISTER_QEMU_OBJECT_INIT_CLASS_SIZED: concrete object with instance_size
 * + free instance_init + free class_init. No class_size override.
 */
#define REGISTER_QEMU_OBJECT_INIT_CLASS_SIZED_NOCS(unique_tag, ClassName,    \
                                                    type_name_str,           \
                                                    parent_type_str,         \
                                                    instance_init_fn,        \
                                                    class_init_fn)           \
...
/*
 * REGISTER_QEMU_OBJECT_CLASS_FINI_SIZED: like REGISTER_QEMU_OBJECT_CLASS_FINI
 * but also sets instance_size = sizeof(ClassName). Useful for types with
 * their own state struct but no member init(), with free instance_finalize
 * and free class_init.
 */
#define REGISTER_QEMU_OBJECT_CLASS_FINI_SIZED(unique_tag, ClassName,         \
                                               type_name_str,                \
                                               parent_type_str,              \
                                               class_init_fn,                \
                                               instance_finalize_fn)         \
...
/*
 * REGISTER_QEMU_OBJECT_INIT_CLASS_SIZED_IFACES: sibling subtype with own
 * state struct (sized) + free instance_init + free class_init + interfaces.
 * Used for runtime-table-driven concrete types that all share the same
 * state struct (e.g. eepro100 variants).
 */
#define REGISTER_QEMU_OBJECT_INIT_CLASS_SIZED_IFACES(                        \
    unique_tag, ClassName, type_name_str, parent_type_str,                   \
...
/*
 * REGISTER_QEMU_OBJECT_CLASS_DATA_IFACES: like REGISTER_QEMU_OBJECT_CLASS_DATA
...
/*
 * REGISTER_QEMU_OBJECT_ABSTRACT_FULL_POST_INIT: abstract object with
 * instance_size + instance_init + instance_post_init + class_size +
 * class_init + interfaces. Used by complex abstract bases (e.g. hostmem)
 * that need a post-init hook on top of regular instance setup.
 */
#define REGISTER_QEMU_OBJECT_ABSTRACT_FULL_POST_INIT(                        \
    unique_tag, ClassName, ClassStruct, type_name_str, parent_type_str,      \
    instance_init_fn, instance_post_init_fn, class_init_fn, ifaces_array)    \
static void unique_tag##_cpp_register_types(void)                            \
{                                                                            \
    static const TypeInfo info = {                                           \
        .name               = type_name_str,                                 \
...
/*
 * REGISTER_QEMU_DEVICE_ABSTRACT_FREE_INIT_CI_NO_CS_IFACES: like
 * REGISTER_QEMU_DEVICE_ABSTRACT_FREE_INIT_CI but with an interfaces
 * array and no class_size (parent's class_size is inherited).
 */
#define REGISTER_QEMU_DEVICE_ABSTRACT_FREE_INIT_CI_NO_CS_IFACES(             \
    ClassName, type_name_str, parent_type_str, instance_init_fn,             \
    class_init_fn, ifaces_array)                                             \
static void ClassName##_cpp_register_types(void)                             \
{                                                                            \
    static const TypeInfo info = {                                           \
...
/*
 * REGISTER_QEMU_OBJECT_INIT_CLASS_FULL: sibling subtype with its own
 * instance_size + class_size, free instance_init + free class_init.
 * Used for concrete subtypes that share state with siblings but need
 * to advertise a specific class struct.
 */
#define REGISTER_QEMU_OBJECT_INIT_CLASS_FULL(unique_tag, ClassName,          \
                                              ClassStruct, type_name_str,    \
                                              parent_type_str,               \
                                              instance_init_fn,              \
                                              class_init_fn)                 \
