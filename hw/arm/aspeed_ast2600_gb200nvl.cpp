...
static void gb200nvl_bmc_i2c_init(AspeedMachineState *bmc)
{
    ...
    I2CBus *i2c[15] = {};
    DeviceState *dev;
    for (size_t i = 0; i < sizeof(i2c) / sizeof(i2c[0]); i++) {
        if ((i == 11) || (i == 12) || (i == 13)) {
            continue;
        }
...
REGISTER_QEMU_OBJECT_CLASS_ONLY_IFACES(aspeed_gb200nvl,
                                        MACHINE_TYPE_NAME("gb200nvl-bmc"),
                                        TYPE_ASPEED_MACHINE,
                                        aspeed_machine_gb200nvl_class_init,
