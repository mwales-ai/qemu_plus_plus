 ...
 */
static void create_uart(MPS3RMachineState *mms, unsigned int uartno, MemoryRegion *mem,
...
void MPS3RMachineState::commonInit(MachineState *machine)
{
    ...
    for (int i = 0; i < (int)ARRAY_SIZE(mms->i2c); i++) {
    ...
    for (int i = 0; i < (int)ARRAY_SIZE(mms->spi); i++) {
    ...
    oscclk = qlist_new();
    for (int i = 0; i < (int)ARRAY_SIZE(an536_oscclk); i++) {
