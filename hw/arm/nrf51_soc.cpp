...
void NRF51State::realize(Error **errp)
{
    DeviceState *dev_soc = DEVICE(this);
    MemoryRegion *mr;
    uint8_t i = 0;
    hwaddr base_addr = 0;
    ...
    qdev_connect_clock_in(DEVICE(&armv7m), "cpuclk", sysclk);
    /*
    ...
    object_property_set_link(OBJECT(&armv7m), "memory", OBJECT(&container),
                             &error_abort);
    ...
    if (!memory_region_init_ram(&sram, OBJECT(this), "nrf51.sram", sram_size,
                                errp)) {
        return;
    }
    memory_region_add_subregion(&container, NRF51_SRAM_BASE, &sram);
    ...
    memory_region_add_subregion_overlap(&container, NRF51_UART_BASE, mr, 0);
    sysbus_connect_irq(SYS_BUS_DEVICE(&uart), 0,
                       qdev_get_gpio_in(DEVICE(&armv7m),
                       BASE_TO_IRQ(NRF51_UART_BASE)));

    /* RNG */
    if (!sysbus_realize(SYS_BUS_DEVICE(&rng), errp)) {
        return;
    }

    mr = sysbus_mmio_get_region(SYS_BUS_DEVICE(&rng), 0);
    memory_region_add_subregion_overlap(&container, NRF51_RNG_BASE, mr, 0);
    sysbus_connect_irq(SYS_BUS_DEVICE(&rng), 0,
                       qdev_get_gpio_in(DEVICE(&armv7m),
                       BASE_TO_IRQ(NRF51_RNG_BASE)));

    /* UICR, FICR, NVMC, FLASH */
    if (!object_property_set_uint(OBJECT(&nvm), "flash-size",
                                  flash_size, errp)) {
        return;
    }

    if (!sysbus_realize(SYS_BUS_DEVICE(&nvm), errp)) {
        return;
    }
    ...
    memory_region_add_subregion_overlap(&container, NRF51_FLASH_BASE, mr, 0);
    ...
    mr = sysbus_mmio_get_region(SYS_BUS_DEVICE(&gpio), 0);
    memory_region_add_subregion_overlap(&container, NRF51_GPIO_BASE, mr, 0);
    ...
    for (i = 0; i < NRF51_NUM_TIMERS; i++) {
        if (!object_property_set_uint(OBJECT(&timer[i]), "id", i, errp)) {
            return;
        }
        ...
        sysbus_mmio_map(SYS_BUS_DEVICE(&timer[i]), 0, base_addr);
        sysbus_connect_irq(SYS_BUS_DEVICE(&timer[i]), 0,
                           qdev_get_gpio_in(DEVICE(&armv7m),
                                            BASE_TO_IRQ(base_addr)));
    }

    /* STUB Peripherals */
    memory_region_init_io(&clock, OBJECT(dev_soc), &clock_ops, NULL,
...
void NRF51State::init()
{
    ...
    object_initialize_child(OBJECT(this), "armv6m", &armv7m, TYPE_ARMV7M);
    qdev_prop_set_string(DEVICE(&armv7m), "cpu-type",
                         ARM_CPU_TYPE_NAME("cortex-m0"));
    qdev_prop_set_uint32(DEVICE(&armv7m), "num-irq", 32);

    object_initialize_child(obj, "uart", &uart, TYPE_NRF51_UART);
    object_property_add_alias(obj, "serial0", OBJECT(&uart), "chardev");

    object_initialize_child(obj, "rng", &rng, TYPE_NRF51_RNG);

    object_initialize_child(obj, "nvm", &nvm, TYPE_NRF51_NVM);

    object_initialize_child(obj, "gpio", &gpio, TYPE_NRF51_GPIO);
...
void NRF51State::classInit(DeviceClass *dc)
{
    device_class_set_props(dc, nrf51_soc_properties);
}

#include "qom/cpp/object.h"
REGISTER_QEMU_DEVICE(NRF51State, TYPE_NRF51_SOC, TYPE_SYS_BUS_DEVICE)
