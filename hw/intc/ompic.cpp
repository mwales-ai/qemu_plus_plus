...
struct OR1KOMPICState {
    ...
    static void ioWrite(void *opaque, hwaddr addr, uint64_t data, unsigned size)
    {
        OR1KOMPICState *s = static_cast<OR1KOMPICState *>(opaque);
        /* We can only write to control control, write control + update status */
        if (OMPIC_REG(addr) == OMPIC_CONTROL) {
    ...
    void realize(Error **errp)
    {
        ...
        if (num_cpus > OMPIC_MAX_CPUS) {
            error_setg(errp, "Exceeded maximum CPUs %d", num_cpus);
            return;
        }
        /* Init IRQ sources for all CPUs */
        for (i = 0; i < num_cpus; i++) {
            sysbus_init_irq(sbd, &cpus[i].irq);
        }
    }
...
const MemoryRegionOps OR1KOMPICState::ompic_ops = {
    .read = OR1KOMPICState::ioRead,
    .write = OR1KOMPICState::ioWrite,
...
const Property OR1KOMPICState::or1k_ompic_properties[] = {
