...
static void qbus_init_internal(BusState *bus, DeviceState *parent,
                               const char *name)
{
    const char *type_name = object_get_typename(OBJECT(bus));
    BusClass *bc;
    int i, bus_id;
    ...
    if (name) {
        ...
        bus->name = g_strdup_printf("%s.%d", type_name, bus_id);
               ...
               DeviceState *parent, const char *name)
{
    ...
    qbus_init_internal(static_cast<BusState *>(bus), parent, name);
}
...
__attribute__((used))
static void bus_class_init(ObjectClass *klass, const void *data)
{
    BusClass *bc = BUS_CLASS(klass);
    ResettableClass *rc = RESETTABLE_CLASS(klass);

    klass->unparent = bus_unparent;
