...
{
    PlatformBusFDTData *data = static_cast<PlatformBusFDTData *>(opaque);
...
{
    return !strcmp(object_get_typename(OBJECT(sbdev)), entry->type_name_);
}

#define TYPE_BINDING(type, add_fn) {(type), NULL, (add_fn), NULL}
/* BindingEntry fields: type_name_, compat, add_fn, match_fn */
                                    ...
                                    hwaddr bus_size, int irq_start)
{
    ...
    qemu_fdt_setprop_cells(fdt, node, "ranges", 0,
