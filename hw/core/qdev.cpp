...
{
    char name[32];
    BusChild *kid = static_cast<BusChild *>(g_malloc0(sizeof(*kid)));
...
bool qdev_unplug_blocked(DeviceState *dev, Error **errp)
{
    if (dev->unplug_blockers) {
        error_propagate(errp, error_copy(static_cast<const Error *>(dev->unplug_blockers->data)));
        return true;
    }
...
__attribute__((used))
static void device_class_base_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc_base = DEVICE_CLASS(klass);

    /* We explicitly look up properties in the superclasses,
     * so do not propagate them to the subclasses.
     */
    dc_base->props_ = NULL;
    dc_base->props_count_ = 0;
}
...
__attribute__((used))
static void device_class_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    VMStateIfClass *vc = VMSTATE_IF_CLASS(klass);
    ResettableClass *rc = RESETTABLE_CLASS(klass);

    klass->unparent = device_unparent;
    ...
    object_class_property_add_bool(klass, "realized",
                                   device_get_realized, device_set_realized);
    object_class_property_add_bool(klass, "hotpluggable",
                                   device_get_hotpluggable, NULL);
    object_class_property_add_bool(klass, "hotplugged",
                                   device_get_hotplugged, NULL);
    object_class_property_add_link(klass, "parent_bus", TYPE_BUS,
...
static const InterfaceInfo device_interfaces[] = {
    { TYPE_VMSTATE_IF },
