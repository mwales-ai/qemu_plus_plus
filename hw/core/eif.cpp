 ...
 */
static bool get_tmp_file(const char *tmpl, char **tmp_path, Error **errp)
{
    int tmp_fd;

    *tmp_path = NULL;
    tmp_fd = g_file_open_tmp(tmpl, tmp_path, NULL);
                            ...
                            uint32_t *crc, Error **errp)
{
    size_t got;
    FILE *tmp_file = NULL;
    uint8_t *kernel = static_cast<uint8_t *>(g_try_malloc(size));
    if (!kernel) {
                             ...
                             Error **errp)
{
    size_t got;
    bool ret = false;
    uint8_t *ramdisk = static_cast<uint8_t *>(g_try_malloc(size));
    if (!ramdisk) {
                                             ...
                                             Error **errp)
{
    ...
    sig = static_cast<uint8_t *>(g_try_malloc(size));
    if (!sig) {
    ...
    *crc = crc32(*crc, sig, size);
    ...
    cert = static_cast<uint8_t *>(g_try_malloc(len));
    if (!cert) {
        error_setg(errp, "Out of memory reading signature section");
        goto cleanup;
    }
    ...
    *kernel_path = *initrd_path = *cmdline = NULL;
    ...
    for (int i = 0; i < eif_header.section_cnt; ++i) {
        ...
        case EIF_SECTION_CMDLINE:
        {
            ...
            *cmdline = static_cast<char *>(g_try_malloc(size + 1));
            if (!*cmdline) {
            ...
            /* other sections including invalid or unknown sections */
        {
            ...
            buf = static_cast<uint8_t *>(g_try_malloc(size));
            if (!buf) {
