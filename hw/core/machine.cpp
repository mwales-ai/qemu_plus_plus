...
HotpluggableCPUList *machine_query_hotpluggable_cpus(MachineState *machine)
{
    ...
    for (i = 0; i < machine->possible_cpus->len; i++) {
        ...
        cpu_item->props = static_cast<CpuInstanceProperties *>(g_memdup(&machine->possible_cpus->cpus[i].props,
                            ...
                            void *opaque, Error **errp)
{
    ...
    SMPConfiguration *config = &config_val;
...
static void machine_initfn(Object *obj)
{
    ...
    BootConfiguration boot_cfg = {};
    machine_copy_boot_config(ms, &boot_cfg);
}
...
static bool is_cpu_type_supported(const MachineState *machine, Error **errp)
{
    ...
    if (mc->valid_cpu_types) {
        ...
        for (i = 0; i < vct->len; i++) {
            if (object_class_dynamic_cast(oc, static_cast<const char *>(vct->pdata[i]))) {
                valid = true;
                break;
            }
        }
        ...
        if (!valid) {
            ...
            error_append_hint(errp, "The valid models are: ");
            for (i = 0; i < vct->len; i++) {
                g_autofree char *model = cpu_model_from_type(static_cast<const char *>(vct->pdata[i]));
                error_append_hint(errp, "%s%s",
                                  model, i + 1 == vct->len ? "\n" : ", ");
            }
        }
