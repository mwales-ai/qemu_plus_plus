                                ...
                                gpointer data)
{
    ...
    if (net_stream_data_send(ioc, condition, d) == G_SOURCE_REMOVE) {
        NetPasstState *s = DO_UPCAST(NetPasstState, data, d);
        Error *error = NULL;
...
{
    NetPasstState *s = static_cast<NetPasstState *>(opaque);
...
static void chr_closed_bh(void *opaque)
{
    ...
    NetClientState *nc_tmp = &s->data.nc;
    net_client_set_link(&nc_tmp, 1, false);
...
static void passt_vhost_user_event(void *opaque, QEMUChrEvent event)
{
    ...
    case CHR_EVENT_OPENED: {
        ...
        NetClientState *nc_link = &s->data.nc;
        net_client_set_link(&nc_link, 1, true);
        s->started = true;
        break;
    }
