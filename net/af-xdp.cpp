...
static void af_xdp_complete_tx(AFXDPState *s)
{
    ...
    for (i = 0; i < done; i++) {
        addr = const_cast<uint64_t *>(reinterpret_cast<const uint64_t *>(xsk_ring_cons__comp_addr(&s->cq, idx++)));
...
static void af_xdp_writable(void *opaque)
{
    AFXDPState *s = static_cast<AFXDPState *>(opaque);
...
static int af_xdp_umem_create(AFXDPState *s, int sock_fd, Error **errp)
{
    ...
    s->buffer = static_cast<char *>(qemu_memalign(qemu_real_host_page_size(), size));
