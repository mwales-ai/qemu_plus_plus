...
void net_checksum_calculate(void *data, int length, int csum_flag)
{
    ...
    if ((size_t)length < sizeof(struct eth_header)) {
        return;
    }
    ...
    if ((size_t)length < sizeof(struct ip_header)) {
        return;
    }
    ...
    case IP_PROTO_TCP:
    {
        ...
        if ((size_t)ip_len < sizeof(tcp_header)) {
            return;
        }
    ...
    case IP_PROTO_UDP:
    {
        ...
        if ((size_t)ip_len < sizeof(udp_header)) {
            return;
        }
                     ...
                     uint32_t iov_off, uint32_t size, uint32_t csum_offset)
{
    ...
    for (i = 0; i < iov_cnt && size; i++) {
        if (iov_off < (iovec_off + iov[i].iov_len)) {
            ...
            uint8_t *chunk_buf = static_cast<uint8_t *>(iov[i].iov_base) + (iov_off - iovec_off);
