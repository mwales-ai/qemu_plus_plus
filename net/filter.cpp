                                    ...
                                    void *opaque)
{
    int ret = 0;
    int direction;
    NetFilterState *nf = static_cast<NetFilterState *>(opaque);
    ...
    next = netfilter_next(nf, static_cast<NetFilterDirection>(direction));
    while (next) {
        /*
         ...
         * pass NULL to next.
         */
        ret = qemu_netfilter_receive(next, static_cast<NetFilterDirection>(direction), sender, flags, iov,
                                     iovcnt, NULL);
        if (ret) {
            return ret;
        }
        next = netfilter_next(next, static_cast<NetFilterDirection>(direction));
    }
...
{
    NetFilterState *nf = NETFILTER(obj);
    nf->direction = static_cast<NetFilterDirection>(direction);
}
...
static const InterfaceInfo netfilter_interfaces[] = {
    { TYPE_USER_CREATABLE },
    { }
};
